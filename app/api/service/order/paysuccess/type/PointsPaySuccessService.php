...
class PointsPaySuccessService extends BaseService
{
    ...
    public function paySuccess($payType, $payData = [])
    {
        ...
        if ($status == true) {
            ...
            // 发送消息通知
            (new MessageService)->pointsPayment($detail);
        }
    }
    ...
    {
        // 更新商品库存、销量
        ProductModel::updateStock($this->model['product_id'], $this->model['total_num']);
        // 整理订单信息
        $order = [
            'pay_type' => $payType,
            'pay_status' => 20,
    ...
    private function updatePayInfo()
    {
        ...
        // 积分兑换扣除用户积分
        if ($this->model['points_num'] > 0) {
            $describe = "用户兑换消费：{$this->model['order_no']}";
            $this->user->setIncPoints(-$this->model['points_num'], $describe);
        }
    }
